...
<jsp:useBean id="pais" scope="request" type="com.example.lab_9.Beans.BPais"/>
        ...
        <div class='container'>
            ...
            <div class="d-flex justify-content-center">
                <div class="w-75">
                    ...
                    <form method="POST" action="<%=request.getContextPath()%>/paises?action=update&id_pais=<%=pais.getIdPais()%>">
                        <div class="form-group">
                            <label for="nombrepais">Nombre</label>
                            <input class="form-control" type="text" disabled name="nombrepais" id="nombrepais" value="<%=pais.getNombre()%>">
                        </div>
                        <div class="form-group">
                            <label class="form-label">Nombre</label>
                            <select class="form-select form-select-sm" name="job_id">
                                <% for (BContinente cont : listacontinentes) { %>
                                <option value="<%=cont.getIdContinente()%>" <%=pais.getContinente().getIdContinente().equals(cont.getIdContinente()) ? "selected" : ""%> ><%=cont.getContinente() %>
                                </option>
                                <% } %>
                            </select>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="text" name="tamanho" id="tamanho" value="<%=pais.getTamanho()%>">
                        </div>

                        <button type="submit" class="btn btn-primary">Enviar</button>
                        <a class="btn btn-danger" href="<%=request.getContextPath()%>/bandas">Cancelar</a>
                    </form>
                </div>
            </div>
        </div>
